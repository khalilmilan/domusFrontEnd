import { deleteSurveyById, fetchParticpantSurveys, fetchSurveys } from '../redux/actions/actionSurvey';
...
const Moments = ({ navigation, route }) => {
    ...
    const [surveyTimeFilter, setSurveyTimeFilter] = useState();
    ...
    const filterSurveysByTime = (timeFilter) => {
        setSurveyTimeFilter(timeFilter);
    ...
    const getSurveyColor = (survey) => {
        let today = new Date();
        if(new Date(survey.startDate) < today && new Date(survey.endDate)>today){
            return "#98FB98";
        }else if(new Date(survey.startDate) > today){
            return "#87CEFA";
        }else if(new Date(survey.endDate) < today){
            return "#FFD700";
        }
    }
    ...
    const applyFilters = () => {
        ...
        let newActiveFilters = [];
        const now = new Date();
        if (surveyTimeFilter == "future") {
            filtered = filtered.filter(survey => new Date(survey.startDate) > now);
            newActiveFilters.push({
                type: 'futurSurveys',
                value: 'hidden',
                label: 'Futur Survey'
            });
        } else if (surveyTimeFilter == "past") {
            filtered = filtered.filter(survey => new Date(survey.endDate) < now);
            newActiveFilters.push({
                type: 'pastSurveys',
                value: 'hidden',
                label: 'Past Survey'
            });
        }else if (surveyTimeFilter == "current") {
            filtered = filtered.filter(survey => 
            new Date(survey.startDate) < now && new Date(survey.endDate) > now
            );
            newActiveFilters.push({
                type: 'currentSurveys',
                value: 'hidden',
    ...
    const renderSurveyItem = ({ item: survey, index }) => (
        <Animated.View
            style={[
        ...
        >
            <TouchableOpacity
                style={[
                    styles.eventCard,
                    { borderLeftColor: getSurveyColor(survey), borderLeftWidth: 4 } // Assurez-vous de définir la largeur pour que la couleur soit visible
                ]}
                ...
                <View style={styles.eventDetails}>
                    <View style={styles.eventInfo}>
                        ...
                        <Text style={styles.eventDate}>
                            from {Formedate(survey.startDate)} to {Formedate(survey.endDate)}
                        </Text>
                    </View>
                    <View style={styles.eventInfo}>
                        <Icon name="group" size={16} color="#666" style={styles.eventIcon} />
                        <Text style={styles.eventCreator}>{survey.groupe.label} </Text>
                    </View>
                </View>
            </TouchableOpacity>
        </Animated.View>
    ...
    const onConfirmeDeleteSurvey = async () => {
         const result = await dispatch(deleteSurveyById(token, selectedSurvey.idSurvey));
        load()
    ...
    const editSurvey = () => {
        setActionVisible(false);
         navigation.navigate('UpdateSurvey', { idSurvey: selectedSurvey.idSurvey, loadList: load }) 
    }
    if (loading) {
        ...
        <View style={styles.container}>
            ...
            >
                ...
                {surveys != undefined &&
                    <FlatList
                        data={filteredSurvey}
                        keyExtractor={(survey) => survey.idSurvey.toString()}
                ...
                <View style={styles.bottomSheetContainer}>
                    ...
                    <View style={styles.radioButtonContainer}>
                        <TouchableOpacity
                            ...
                            onPress={() => filterSurveysByTime('past')}
                        >
                            <View style={styles.radioCirclePast}>
                                {surveyTimeFilter === 'past' && <View style={styles.selectedRbPast} />}
                            </View>
                            <Text style={styles.radioText}>Past Surveys</Text>
                        </TouchableOpacity>
                         <TouchableOpacity
                            style={styles.radioButton}
                            onPress={() => filterSurveysByTime('current')}
                        >
                            <View style={styles.radioCircleCurrent}>
                                {surveyTimeFilter === 'current' && <View style={styles.selectedRbCurrent} />}
                            </View>
                            <Text style={styles.radioText}>Current Surveys</Text>
                        </TouchableOpacity>
                        
                        <TouchableOpacity
                            style={styles.radioButton}
                            onPress={() => filterSurveysByTime('future')}
                        >
                            <View style={styles.radioCircleFutur}>
                                {surveyTimeFilter === 'future' && <View style={styles.selectedRbFutur} />}
                            </View>
                            <Text style={styles.radioText}>Future Surveys</Text>
                        </TouchableOpacity>
                        <TouchableOpacity
                            style={styles.radioButton}
                            onPress={() => filterSurveysByTime('all')}
                        >
                            <View style={styles.radioCircle}>
                                {surveyTimeFilter === 'all' && <View style={styles.selectedRb} />}
                            </View>
                    ...
                    <View style={styles.modalBackground}>
                        <View style={styles.modalContainer}>
                            ...
                            <Text style={styles.modalText}>
                                {selectedSurvey != null && `Do you really want to delete ${selectedSurvey.title}?`}
                            </Text>
                            <View style={styles.buttonContainer}>
                                <TouchableOpacity style={styles.cancelButton} onPress={() => setModalVisible(false)}>
                                    <Text style={styles.buttonTextModal}>Cancel</Text>
                                </TouchableOpacity>
                                <TouchableOpacity style={styles.confirmButton} onPress={onConfirmeDeleteSurvey}>
