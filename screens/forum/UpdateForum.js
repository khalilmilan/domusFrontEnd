...
const UpdateForum = ({ navigation, route }) => {
    ...
    let loadForumList = route.params.loadForumList
   const [formData, setFormData] = useState({
        idForum:forum.idForum,
        title: forum.title,
        description: forum.description,
        idEvent:forum.idEvent,
        idUser:forum.idUser
    });
    ...
    const handleSubmit = async () => {
        try {
           ...
           if (!formData.title.trim()) {
            ...
            showAlertWithMessage('Forum Updated successfully!', 'success');
            try {
                let userDetails = await AsyncStorage.getItem("userDetails");
                if (userDetails != null) {
                    let user = JSON.parse(userDetails);
                    let token = user.token;
                    const uiUpdate = await dispatch(updateForumm(token, formData));
                    loadForumList();
                    setTimeout(() => {
                        navigation.navigate('ListForum', { eventId: formData.idEvent })
                    }, 1500);
                } else {
            ...
            console.error(error);
            Alert.alert('Erreur', 'Impossible de mettre à jour le forum');
        }
    };
 ...
 return (
        <View style={styles.container}>
            ...
            <Text style={styles.formTitle}>Edit Forum</Text>
